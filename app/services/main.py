...
def create_app(config_class: Optional[object] = None) -> Flask:
    ...
    ffmpeg_paths = [
        ...
        os.path.join(
            os.path.dirname(__file__), "..", "..", "bin", "ffmpeg.exe"
        ),  # Relatif à l'app
        "ffmpeg",  # Dans le PATH
    ]
    ...
    for ffmpeg_path in ffmpeg_paths:
        if (
            ...
            or os.system(f"where {ffmpeg_path} > nul 2>&1") == 0
        ):
    ...
    if not app.debug:
        ...
        try:
            file_handler = RotatingFileHandler(
                log_file, maxBytes=1024 * 1024, backupCount=10, delay=True  # 1MB
            )  # Ouverture différée du fichier
