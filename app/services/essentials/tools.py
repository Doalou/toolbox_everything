...
class QRCodeGenerator:
    ...
    def generate(
        self, text: str, size: int = 10, border: int = 4, error_correction: str = "M"
    ) -> str:
...
class TextProcessor:
    ...
    def format_text(self, text: str, format_type: str) -> str:
        ...
        if format_type == "uppercase":
            ...
            return ". ".join(
                s.strip().capitalize() for s in text.split(".") if s.strip()
            )
...
class ColorPaletteGenerator:
    ...
    def generate(
        self, base_color: str, palette_type: str, count: int
    ) -> List[Dict[str, str]]:
    ...
    def _generate_monochromatic(
        self, base_hsv: tuple, count: int
    ) -> List[Dict[str, str]]:
        colors = []
        h, s, v = base_hsv
    ...
    def _generate_complementary(
        ...
        rgb = tuple(int(c * 255) for c in rgb)
        colors.append(
            {"hex": self._rgb_to_hex(rgb), "rgb": f"rgb({rgb[0]}, {rgb[1]}, {rgb[2]})"}
        )
...
class URLValidator:
    ...
    def __init__(self, allowed_domains=None, allowed_domain_prefixes=None):
        try:
            ...
            self.allowed_domains = current_app.config.get(
                "URL_VALIDATOR_ALLOWED_DOMAINS", self.DEFAULT_ALLOWED_DOMAINS
            )
            self.allowed_domain_prefixes = current_app.config.get(
                "URL_VALIDATOR_ALLOWED_DOMAIN_SUFFIXES",
                self.DEFAULT_ALLOWED_DOMAIN_PREFIXES,
            )
        except RuntimeError:
            self.allowed_domains = allowed_domains or self.DEFAULT_ALLOWED_DOMAINS
            self.allowed_domain_prefixes = (
                allowed_domain_prefixes or self.DEFAULT_ALLOWED_DOMAIN_PREFIXES
            )
    ...
    def _resolve_domain_safely(self, domain: str) -> Dict[str, Any]:
        try:
            ...
            return {"safe": True, "resolved_ips": ip_addresses}
    ...
    def validate_and_analyze(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                ...
                "security_check": {"passed": False, "reasons": []},
            }
            ...
            if parsed.scheme not in self.ALLOWED_SCHEMES:
                analysis["security_check"]["reasons"].append(
                    f"Scheme non autorisé: {parsed.scheme}"
                )
                return analysis
            ...
            try:
                ...
                response = requests.head(
                    url, timeout=5, allow_redirects=True, headers=headers, verify=True
                )
                ...
                if response.url != url:
                    ...
                    if not self._is_domain_allowed(final_domain):
                        analysis["status"][
                            "security_warning"
                        ] = f"Redirection vers un domaine non autorisé: {final_domain}"
...
class JSONFormatter:
    ...
    def process(self, json_text: str, operation: str = "format") -> Dict[str, Any]:
        try:
            ...
            result = {"is_valid": True, "original": json_text}
    ...
    def _analyze_structure(self, obj: Any, depth: int = 0) -> Dict[str, Any]:
        if isinstance(obj, dict):
            return {
                ...
                "depth": depth,
                "properties": {
                    k: self._analyze_structure(v, depth + 1) for k, v in obj.items()
                },
            }
            ...
            return {"type": type(obj).__name__, "depth": depth, "value": str(obj)[:100]}
