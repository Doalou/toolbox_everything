...
def get_format_string(quality: str) -> str:
    ...
    return quality_map.get(
        quality, "best[height>=2160]/best[height>=1440]/best[height>=1080]/best"
    )
    ...
    try:
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            if info is None:
                return (
                    jsonify(
                        {"error": "Impossible d'obtenir les informations de la vidéo"}
                    ),
                    400,
                )
        ...
        current_app.logger.error(
            f"Erreur lors de l'extraction des infos YouTube: {error_msg}"
        )
        ...
        if "Video unavailable" in error_msg or "Private video" in error_msg:
            return (
                jsonify(
                    {
                        "error": "Cette vidéo n'est pas accessible (privée, supprimée ou géo-restreinte)"
                    }
                ),
                400,
            )
            ...
            return (
                jsonify({"error": "Cette vidéo nécessite une vérification d'âge"}),
                400,
            )
        else:
...
def download_video():
    ...
    current_app.logger.info("=== DEBUG QUALITÉ AMÉLIORÉ ===")
    ...
    try:
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            if not info:
                return (
                    jsonify(
                        {"error": "Impossible d'extraire les informations de la vidéo"}
                    ),
                    400,
                )
            ...
            current_app.logger.info(
                f"Formats disponibles: {len(info.get('formats', []))}"
            )
            ...
            if "formats" in info:
                ...
                for i, fmt in enumerate(video_formats[:10]):
                    current_app.logger.info(
                        f"{i+1}. ID: {fmt.get('format_id', 'N/A')}, "
                ...
                current_app.logger.info(
                    f"Format sélectionné par yt-dlp: {selected_format}"
                )

            # Maintenant télécharger avec le format optimal
            current_app.logger.info(
                f"Démarrage du téléchargement avec format: {format_string}"
            )
            ...
            current_app.logger.info(
                f"Taille du fichier: {file_size} bytes ({file_size / (1024*1024):.2f} MB)"
            )
            current_app.logger.info("=== FIN DEBUG QUALITÉ AMÉLIORÉ ===")
        ...
        if "requested format not available" in error_msg.lower():
            return (
                jsonify({"error": "Format demandé non disponible pour cette vidéo"}),
                400,
            )
...
def test_dependencies():
    ...
    result = {"yt_dlp": False, "ffmpeg": False, "errors": []}
