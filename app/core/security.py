...
class SecurityManager:
    ...
    def validate_file_content(
        cls, file_path: str, expected_extension: str = None
    ) -> bool:
            ...
            if actual_mime_type.startswith("text/"):
                ...
                with open(file_path, "rb") as f:
                    ...
                    if b"\x00" in chunk or any(
                        b < 0x09 or (0x0E <= b <= 0x1F) for b in chunk
                    ):
                        raise SecurityError(
                            "Fichier texte contenant des données binaires suspectes"
                        )
        ...
        try:
            ...
            if file_size > max_size_bytes:
                raise ValidationError(
                    f"Fichier trop volumineux: {file_size} bytes (max: {max_size_bytes} bytes)"
                )

            return True
        except OSError as e:
            raise SecurityError(
                f"Erreur lors de la vérification de la taille: {str(e)}"
            )

    @classmethod
    def comprehensive_file_validation(
        cls, file_path: str, allowed_extensions: set = None, max_size_mb: int = 100
    ) -> Dict[str, Any]:
            ...
            try:
                if validate_content:
                    ...
                    if not validation_result["valid"]:
                        raise SecurityError(
                            f"Fichier non valide: {'; '.join(validation_result['errors'])}"
                        )
                    ...
                    if allowed_extensions:
                        SecurityManager.validate_file_extension(
                            secure_name, allowed_extensions
                        )
